...
export default function ExtractTableFromImage() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const filename = `extracted-tables-${format}.zip`;
  ...
  const handleTableExtraction = async () => {
    ...
    try {
      setStatus(`Extracting tables from Image...`);
      const response = await apiService.imageToTableData(file, format, mimetype);
      ...
      setStatus(err.response?.data?.error || "Failed to extract tables");
      setTableCount(0);
      alert(err.response?.data?.error || err.message || "Error extracting tables");
    }
  };
    ...
    <PageLayout>
      <NavBar />
        <div className="p-4 flex flex-col items-center" style={{border: "2px solid #000", borderRadius: "15px", padding: "10px"}}>
          ...
          <button id="btn_extract" style={{margin: '10px'}} onClick={handleTableExtraction} disabled={cooldown>0}>Extract Table Data</button>
          {status && <p className="mb-2" style={{color: 'green', fontWeight: 'bold'}}>{status}</p>}
        </div>
        {tableCount > 0 && (
        <div className="flex flex-col items-center">
          <p style={{color: 'green', fontWeight: 'bold'}}>Extracted Table Count : {tableCount}</p>
          <button
